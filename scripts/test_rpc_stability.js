...
async function run() {
  ...
  console.log(`Found perfume: ${perfumeId}. Running 5 consecutive RPC calls...`);
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (rpcError) {
      console.error(`Call ${i}: Error:`, rpcError);
    } else {
      console.log(`Call ${i}: Success (${rpcData.length} items) in ${duration}ms`);
    }
  }
}
